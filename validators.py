...
def validate_and_fix(rec: Dict, part_number: str) -> Tuple[Dict, List[str]]:
    ...
    # ---- 3. L/W/H：通用 sanity check（V8 拔掉 SMD 二極體 overfit 0.2~15mm）----
    # 改成 0.1~100mm 涵蓋 SOD 到 TO-247 / TO-220 / SO-IC 等所有常見封裝
    for field in ["Maximum Length (mm)", "Maximum Width (mm)", "Maximum Height (mm)"]:
        v = rec.get(field)
        if isinstance(v, (int, float)) and (v > 100 or v < 0.1):
            fixes.append(f"{field}={v} 超出 0.1~100mm → null")
            rec[field] = None
    ...
    # ---- 5. PIN：V8 拔掉 {2,3,4,8} overfit，改通用 sanity（> 0 即可）----
    # 原規則對 SOD/SOT diodes work 但對 SO-IC (14/16) / TO-220 (3/5) 等會誤砍
    pin = rec.get("PIN Number")
    if isinstance(pin, (int, float)) and (int(pin) < 1 or int(pin) > 200):
        fixes.append(f"PIN={pin} 超出 1~200 範圍 → null")
        rec["PIN Number"] = None
