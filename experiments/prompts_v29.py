...
- **Tj 優先規則**：datasheet 若給 Tj **任何形式**（range 或 single max value），**永遠**優先用 Tj 而非 Tstg
- ✓ datasheet 句型 "Tj = <lo> to <hi>°C" → 填 `<lo>`(通常為負整數)
- 若 datasheet 把 Tj 與 Tstg 併寫成 "Tj/Tstg = a to b"，視為 Tj 範圍
- **重要 fallback（須優先檢查）**：若 datasheet 給 **Tj 是 single max value**（例 `Tj = 150°C max` 或表內 Tj 欄只一個數字 150），**即使 Tstg 是 range（例 Tstg = -55 to 150）**，Min Operating Temp **必須填 Tj 的 single value**（規格上限即下限的特殊定義），**禁止退而求其次填 Tstg 的低端**
...
## 8. I_O、I_F (A) — 浮點
- **優先級**：**I_O (Average Rectified Output Current) > I_F(AV) > Forward Continuous I_F > Peak I_F**
- 若 datasheet 同時給 I_O 跟 I_F（peak 或 continuous），**必取 I_O**
- ✓ datasheet 同時寫 "I_F = 300mA (Forward Continuous), I_O = 150mA" → 填 `0.15`（I_O 優先）
- ✓ 只給 I_F (continuous) 沒 I_O → 填 I_F 值（換算到 A）
- ✓ 只給 Peak I_FM 沒 average → 用 Peak I_FM 值
- **單位一律 A**，mA 必須 ÷1000
- ✓ datasheet 寫 "I_O = <M> mA" → 填 `<M>/1000`（範例：`150mA` → `0.15`）
- ✗ datasheet 同時有 I_O 跟 I_F 時抽 I_F — I_O 才是規格量
