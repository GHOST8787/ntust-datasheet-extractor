"""整合:把「裁切 + 四條通用規則」版尺寸抽取替換進 V23,跑完整 110 格印確切整體 + 逐欄。
零 overfit。不估,實跑。
"""
...
out_dir = config.ARCHIVE_DIR / "v_clean_dim_rules_integrated"
...
for pn, exp in spec.items():
    for f in config.FIELDS:
        ...
        correct += int(ok)
        per_field.setdefault(f, 0)
        per_field[f] += int(ok)

print(f"\n=== 整合後完整 110 格(零 overfit:三模型投票 + 裁切 + 四條通用規則)===", flush=True)
