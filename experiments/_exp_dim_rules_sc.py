"""零 overfit:單步四條規則 + 同顆跑 N 次取多數(self-consistency),壓 VLM 隨機飄。
不分步(跟退步的 v2 不同),只在「單步 rules prompt」外面包多次取多數。
測全 10 顆尺寸三欄,對照 rules 單次 26/30。
"""
...
print(f"尺寸抽取 — 裁切 + 四條規則 + 多次取多數 x{N_SAMPLES}(零 overfit)— 全 10 顆\n", flush=True)
hit = 0
total = 0
...
print(f"\n尺寸格命中: {hit}/{total} = {hit/total*100:.0f}%  (rules 單次: 26/30)", flush=True)
